...
class Command(pdu.PDU):
    ...
    def generate_params(self):
        ...
        for field in self.params_order:
            try:
                ...
                logging.debug('generate_params field {} error: {}'.format(field,str(e)))
            logging.debug(field + ":" + str(value))
            #print value
        return body
    ...
    def _generate_int(self, field):
        ...
        if data:
            ...
            #    return chr(data)
            #else:
            #    return struct.pack(fmt, data).decode('utf-8')
            return struct.pack(fmt, data)
        else:
            return struct.pack('B', 0)  # null terminator
    ...
    def _generate_string_tlv(self, field):
        ...
        if hasattr(self.params[field], 'size'):
            ...
            value = struct.pack(">HH", field_code, size) + fvalue.encode('utf-8')
        elif hasattr(self.params[field], 'max'):
            if len(field_value or '') > self.params[field].max:
                field_value = field_value[0:self.params[field].max - 1]

            if field_value:
                field_length = len(field_value)+1
                fvalue = field_value + chr(0)
                value = struct.pack(">HH", field_code, field_length) + fvalue.encode('utf-8')
                #print binascii.b2a_hex(value)
            else:
                value = None  # chr(0)
        #setattr(self, field, field_value)
    ...
    def _parse_string(self, field, data, pos):
        ...
        end = data.find(chr(0).encode('utf-8'), pos)
        length = end - pos

        field_value = (data[pos:pos + length]).decode('utf-8')
        setattr(self, field, field_value)
        pos += length + 1
    ...
    def _parse_ostring(self, field, data, pos, length=None):
        ...
        setattr(self, field, data[pos:pos + length].decode('utf-8'))
        pos += length
...
class SubmitSM(Command):
    ...
    def prep(self):
        ...
        if self.short_message:
            ...
            logging.debug('message length:'+str(self.sm_length))
