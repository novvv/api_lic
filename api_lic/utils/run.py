...
def my_worker():
    ...
    args = '-q -b redis://localhost:6379/0 -E -A api_lic.task worker -l info -n worker.{}'.format(name).split(' ')
    with redirect_argv(args):
        celery()

def my_beat():
    args = '-q -b redis://localhost:6379/0 -A api_lic.task beat -l info'.split(' ')
    with redirect_argv(args):
        celery()
